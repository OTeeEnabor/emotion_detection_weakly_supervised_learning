...
def compute_batch_loss(logits, noisy_labels, P):
  ...
  unobserved_mask = (noisy_labels == 0)
  ...
  # after the first epoch
    ...
    # if the method is large loss permanent correction,  calculate weight using delta rel
    if P['mod_scheme'] == 'LL-Cp':
      k = tensorflow.math.ceil(batch_size * num_classes * P['delta_rel'])
    # if the method is not permanent large loss correction use the following to calulate weight
    else:
    ...
    unobserved_loss = tensorflow.math.multiply(unobserved_mask,
                                               loss_matrix.numpy().reshape([batch_size, -1]))  # .numpy().reshape([32,-1])
    flatten_unobserved_loss = tensorflow.reshape(unobserved_loss, [-1])
    print(f'k is {k}...and k.shape is {k.shape}')
    ...
    if P['mod_scheme'] in ['LL-Ct', 'LL-Cp']:
      final_loss_matrix = tensorflow.where(unobserved_loss < topk_lossvalue, loss_matrix.numpy().reshape([batch_size, -1]),
...
# load model
MODEL_PATH = '/home-mscluster/oenabor/home/wsml/exp1/saved_models/00004-test-train-08122022/model-00-0.99.hdf5'
model = load_model(MODEL_PATH)
# load test data
X_test_path = '/home-mscluster/oenabor/home/wsml/data/X_val.npy'
X_test = np.load(X_test_path, allow_pickle=True)

y_val_path = '/home-mscluster/oenabor/home/wsml/data/y_val.npy'
# load true labels
...
np.save('/home-mscluster/oenabor/home/wsml/exp1/saved_models/00004-test-train-08122022/predict_test',y_pred)
print('predictions saved into numpy')

compute_metrics(y_val,y_pred,'/home-mscluster/oenabor/home/wsml/exp1/saved_models/00004-test-train-08122022/',1)
