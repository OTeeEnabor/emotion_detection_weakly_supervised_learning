...
def pseudo_train(pre_model,X_labelled,Y_labelled, X_unlabelled, P, run_dir,X_test,Y_test):
  ...
  labelled_numUpdates = int(X_labelled.shape[0]/P["batch_size"])
  ...
  pseudo_labels = pre_model.predict(pseudo_x)
  ...
  step = P['step']
  for epoch in range(0, P["num_epochs"]):
    pseudo_label_distribution = np.zeros((X_unlabelled.shape[0], P["num_classes"]))
    for i in range(0, pseudo_numUpdates):
      ...
      pseudo_label_distribution[start: end] = pseudo_labels
      if i % 50 ==0:
        for k in range(0, labelled_numUpdates):
          with tensorflow.GradientTape() as labelled_tape:
            labelled_predict = pre_model(x_label, training=True)
            main_loss = loss_fn(y_label, labelled_predict)
          grads_labelled = labelled_tape.gradient(main_loss,pre_model.trainable_variables)
          opt.apply_gradients(zip(grads_labelled, pre_model.trainable_variables))
          x_label, y_label = next(iter(labelled_datasaet))
    ...
    predict_test = pre_model.predict(X_test)
    predict_test = label_binarizer(predict_test,0.9)
    test_acc  = accuracy_score(y_true=Y_test, y_pred = predict_test)
    test_f1 = f1_score(y_true=Y_test, y_pred=predict_test, average='weighted')
    Y_test = tensorflow.cast(Y_test, dtype=tensorflow.float32)
    predict_test = tensorflow.cast(predict_test, dtype=tensorflow.float32)
    test_loss = loss_fn(Y_test, predict_test)
    # save pseudo_distribution
    pseudo_label_distribution_df =pd.DataFrame(pseudo_label_distribution)
    pseudo_label_distribution_df.to_csv(f'{run_dir}/pseudo_label_distribution-epoch-{epoch}.csv')
    ...
    loss_pseudo_list.append(pseudo_loss.numpy())
    loss_combined_list.append(main_loss.numpy())
    loss_val_list.append(test_loss.numpy())
  ...
  train_dictionary = {
    ...
    "pseudo loss": loss_pseudo_list,
    "val loss": loss_val_list
  }
  train_df = pd.DataFrame(train_dictionary)
  train_df.to_csv(f'{run_dir}/train_semi_pseudo_3_info.csv')
...
img_data = pd.read_csv('/home-mscluster/oenabor/home/au_models/data/img_data_norm.csv')
img_data = img_data.sample(frac=0.2,random_state=1)
img_data.to_csv(f'{run_dir}/training_sample.csv')
# change dtypes to number
img_data[['AU1',"AU2","AU4","AU5","AU6","AU9","AU12","AU15","AU17","AU20","AU25","AU26"]] = img_data[['AU1',"AU2","AU4","AU5","AU6","AU9","AU12","AU15","AU17","AU20","AU25","AU26"]].apply(pd.to_numeric)

# create the dataset
X_train,y = create_dataset(img_data)
# load true labels
