...
def pseduo_train(pre_model,X_unlabelled, X_labelled, Y_labelled, run_dir, P):
  """
  :param pre_model:
  :param X_unlabelled:
  :param X_labelled:
  ...
  :param run_dir:
  :param P:
  :return: pre-trained model, training log
  """
  avg_main_loss = tensorflow.keras.metrics.Mean(name='avg_main_loss', dtype=tensorflow.float32)
  labelled_dataset = tensorflow.data.Dataset.from_tensor_slices((X_labelled,Y_labelled))
  labelled_dataset = labelled_dataset.shuffle(1000).batch(P["batch_size"])
  ...
  opt = tensorflow.keras.Optimizers.Adam(learning_rate=P["lr"])
  # use model to generate pseudo-labels for unlabelled dataset
  pseudo_labels = pre_model.predict(X_unlabelled)
  # use threshold to "get most confident"
  pseudo_labels = label_binarizer(pseudo_labels,0.5)
  # define loss function
  loss_fn = tensorflow.keras.losses.BinaryCrossentropy()
  # define number of uptates
  pseudo_numUpdates  = int(X_unlabelled.shape[0]/P["batch_size"])
  # labelled images batch
  x_labelled, y_labelled = next(iter(labelled_dataset))
  # unlabelled image batch
  pseudo_x = next(iter(pseudo_dataset))
  # predict pseudo_labels
  pseudo_labels = pre_model.predict(pseudo_x)
  # change confident values based on threshold
  pseudo_labels = label_binarizer(pseudo_labels,0.5)
  for epoch in range (0, P["num_epochs"]):
    for i in range(0,pseudo_numUpdates):
      with tensorflow.GradientTape() as pseudo_tape:
        print(f'\n Epoch {epoch}: make prediction: {i}')
        # use model to predict on labelled data
        labelled_predict  = pre_model(x_labelled, training=True)
        # use model to predict on unlabelled data
        pseudo_label_predict = pre_model(pseudo_x, training=False)
        # calculate labelled loss
        labelled_loss = loss_fn(y_labelled, labelled_predict)
        # calculate pseudo-loss
        pseudo_loss = alpha_weight(P["step"]) * loss_fn(pseudo_labels[pseudo_labels, pseudo_label_predict)
        loss = labelled_loss + pseudo_loss

      # get next iteration of data
      x_labelled, y_labelled = next(iter(labelled_dataset))
      # get next iteration of unlabelled images
      pseudo_x = next(iter(pseudo_dataset))
      pseudo_labels = pre_model.predict(pseudo_x)
      # change confident values based on threshold
      pseudo_labels = label_binarizer(pseudo_labels, 0.5)
...
# load true labels
y_labelled = np.load(y_path, allow_pickle=True)
