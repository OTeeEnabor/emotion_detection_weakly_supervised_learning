...
def er_label_encoder(y):
  ...
  return dummy_y
def encoder_pseudo(arr):
  labels = ['anger', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise']
  new_arr = []
  for i in arr:
    new_arr.append(labels[i])
  return er_label_encoder(new_arr)
...
def pseudo_train(pre_model,X_labelled,Y_labelled, X_unlabelled, P, run_dir,X_test,Y_test):
  ...
  pseudo_labels = np.array([np.argmax(x) for x in pseudo_labels])# njvbglo/kjn
  pseudo_labels = encoder_pseudo(pseudo_labels)#label_binarizer(pseudo_labels, 0.5)
  print(f'pseudo labels shape -{pseudo_labels}')
  ...
  for epoch in range(0, P["num_epochs"]):
    ...
    for i in range(0, pseudo_numUpdates):
      ...
      print(f'\n Epoch {epoch}: make prediction: {i}- step:{step}')
      with tensorflow.GradientTape() as pseudo_tape:
        ...
        print(f'pseudo predict shape -{pseudo_predict}')
        main_loss = loss_fn(y_label, labelled_predict)
        pseudo_loss = alpha_weight(step) * loss_fn(pseudo_labels,pseudo_predict)
      ...
      pseudo_labels = encoder_pseudo(pseudo_labels)
      #label_binarizer(pseudo_labels, 0.5)
    ...
    predict_test = np.array([np.argmax(x) for x in pseudo_labels])#label_binarizer(predict_test,0.5)
    predict_test = encoder_pseudo(predict_test)
    ...
    print(f'Epoch {epoch} performance on test: alpha-weight- {alpha_weight(step)} test_accuracy-{test_acc} - test_f1: {test_f1} ')
    ...
    loss_combined_list.append(main_loss)
  ...
  train_df = pd.DataFrame(train_dictionary)
  train_df.to_csv(f'{run_dir}/pseduo_er_train_info.csv')
###############################################
...
X_path = '/home-mscluster/oenabor/home/pseudo/exp1/datasets/ER_data/X_train.npy'
y_path = '/home-mscluster/oenabor/home/pseudo/exp1/datasets/ER_data/y_train.npy'
# load data numpy files
X_train = np.load(X_path, allow_pickle=True)
# load true labels
y = np.load(y_path, allow_pickle=True)
y_train = er_label_encoder(y) #label_binarizer(y,0.5)
...
# load test data
X_test_path = '/home-mscluster/oenabor/home/pseudo/exp1/datasets/ER_data/X_val_pseudo.npy'
X_test = np.load(X_test_path,allow_pickle=True)
Y_test_path = '/home-mscluster/oenabor/home/pseudo/exp1/datasets/ER_data/y_val_pseudo.npy'
Y_test = np.load(Y_test_path, allow_pickle=True)
y_test = er_label_encoder(Y_test)
# y_true_df = pd.DataFrame(y,columns=["Emotion"])
# y_test = y_true_df['Emotion']
# y_test =
