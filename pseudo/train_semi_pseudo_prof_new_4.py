...
def pseudo_train(pre_model,X_labelled,Y_labelled, X_unlabelled, P, run_dir,X_test,Y_test):
  ...
  pseudo_labels = pseduo_label_binarizer(pseudo_labels)
  ...
  for epoch in range(0, P["num_epochs"]):
    ...
    predict_test = label_binarizer(predict_test,0.5)
    ...
    print(f'Epoch {epoch} performance on test: alpha-weight- {alpha_weight(P["step"])} test_accuracy-{test_acc} - test_f1: {test_f1} ')
  ...
  train_df = pd.DataFrame(train_dictionary)
  train_df.to_csv(f'{run_dir}/train_info_prof4.csv')
